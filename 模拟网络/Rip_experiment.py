...
if __name__ == '__main__':
    ...
    for episode in range(25):
        ...
        for i in range(10):
            ...
            throughput_set.append(min(rip_network.get_net_state().values()))
            for j in range(cache_size):
                rip_network.update_dataset(is_create_data=False)
                throughput_set.append(min(rip_network.get_net_state().values()))
        while True:
            rip_network.update_dataset(is_create_data=False)
            state = [item for item in rip_network.get_net_state().values()]
    ...
    logging.info(f'RIP：每创建一次数据包后单纯发送的次数:{cache_size},丢包率:{np.around(np.average(loss_sets), 6)}\n'
                 f'数据包发送速率:{rip_network.data_number}Gbps,数据包的大小:{rip_network.data_size}Kb'
