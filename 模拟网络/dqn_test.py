...
if __name__ == '__main__':
    ...
    for j in range(25):
        ...
        """准备进行网络拓扑中信息的传输!"""
        thread_pool = []  # 线程池
        ospf_net.time = time.perf_counter()  # 更新网络开始时的时间戳，为计算吞吐量做准备
        """正式启动信息发送"""
        for data in ospf_net.data_set:
            ...
            path = max([_item for _item in values.items()],
                       key=lambda x: x[0])[1]
            task_thread = threading.Thread(target=ospf_net.send_message,
        ...
        gross = 0  # 数据包总量
        failure = 0  # 成功的数据包数量
        for log in ospf_net.logs.values():
            gross += 1
            if not log[-1]:
                failure += 1
        print(f'丢包率:{round(failure / gross * 100, 3)}%')  # 保留三位小数
        average_loss.append(round(failure / gross * 100, 3))
        ospf_net.logs.clear()  # 每一轮发送信息之后清空记录。
        ospf_net.update_dataset(True)

    print(f'这次数据包的起止大小:{(ospf_net.size_min, ospf_net.size_max)}。使用DQN算法的平均丢包率:{np.mean(average_loss)}%')
    print(f'这次数据包的起止大小:{(ospf_net.size_min, ospf_net.size_max)}。使用DQN算法的丢包率的集合:{average_loss}')
